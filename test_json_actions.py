...
def initialize(rule_engine):
    json = { "name" : "alarm_rule_template","scope" : {"deviceType" : "{{rule.deviceType}}"},"condition" : { "expression" :  "{{telemetry.messageId}} = {{rule.messageId}}"},"actions" : [{"action" :  "SEND_EMAIL","data" : {"emailId":"{{rule.emailId}}","messageId":"{{rule.messageId}}","deviceType":"{{rule.deviceType}}"}}], "variable_metadata" : [{"name" : "rule.deviceType","type" : "string" },{"name" : "telemetry.messageId","type" : "string"},{"name" : "rule.messageId","type" : "string"}]}
